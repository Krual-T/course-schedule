...
async function calculatePKCECodeChallenge(verifier) {
...
export class FeishuClient {
    ...
    async exchangeCodeForToken(code) {
        try {
            ...
            const response = await fetch(`/proxy-api/feishu/exchange-token`, {
                method: 'POST',
            ...
            if (responseData.code !== 0) {
                throw new Error(`兑换user_access_token失败: ${responseData.msg || '未知错误'}`);
            }

            // 4. 存储用户令牌到本地
            const now = Date.now() / 1000; // 当前时间（秒级）
            this.userTokenInfo = {
                accessToken: responseData.access_token,  // 飞书用户access_token
                refreshToken: responseData.refresh_token, // 飞书用户refresh_token
                expireTime: now + responseData.expires_in // 令牌过期时间（秒级）
            };
    ...
    async refreshUserToken() {
        ...
        try {
            ...
            const response = await fetch(`/proxy-api/feishu/refresh-token`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (responseData.code !== 0) {
                throw new Error(`刷新user_access_token失败: ${responseData.msg || '未知错误'}`);
            }
            ...
            this.userTokenInfo = {
                ...
                expireTime: now + responseData.data.expires_in
            };
            this._saveUserTokenToStorage();
    ...
    async request(method, path, data = null, params = null) {
        try {
            ...
            if (responseData.code !== 0) {
                ...
                if ([901001, 901002, 901003, 901004].includes(responseData.code)) {
                    this.clearUserToken();
                    throw new Error(`令牌无效，请重新授权（错误码: ${responseData.code}）`);
                }
                throw new Error(`API调用失败: ${responseData.msg} (错误码: ${responseData.code})`);
            }

            return responseData.data; // 返回飞书API的data字段（核心数据）
        } catch (error) {
                        throw error;
        }
    }
     ...
     */
    async getCourseInfoTableMetadata(openId = null, weekNumber = null) {
        ...
        try {
            ...
            // 如果提供了openId，添加过滤条件
            if (openId) {
                // 使用传入的周数，如果没有传入则使用当前周
                const targetWeek = weekNumber !== null ? weekNumber : courseSchedule.getCurrentWeek();
